...
export const TermCard = ({
  ...
  return (
    <Link
      href={`${baseUrl}/${id}`}
      className='h-48 space-y-3 bg-bg-100 p-4 rounded-sm shadow-md flex flex-row justify-start items-start hover:bg-bg-200'
      title={`Proyectos de ${title}`}
    >
      <div className='w-2/6 h-hull mx-auto text-primary-300 flex items-center justify-center'>
        {icon}
      </div>
      <div className='flex flex-col justify-normal items-start w-2/3 text-left pl-1'>
        ...
        <h4 className='text-bse text-gray-700 font-semibold'>
          {subTitle ?? ''}
        </h4>
      </div>
    </Link>
